...
from pyJianYingDraft import TextIntro, TextOutro,AudioSceneEffectType, Text_loop_anim, Mask_type, VideoSceneEffectType, animation, IntroType, OutroType, Transition_type, trange, GroupAnimationType, TransitionType
...
class autoCut():
    def __init__(self,topic: str = "", title: str = "", wenan: str = ""):
        self.topic = topic
        self.title = title
        self.wenan = wenan
        ...
        self.script.add_track(draft.TrackType.video, 'SY')
    ...
    def addTitle(self,title: str = "", wenan: str = ""):
        if title and wenan:
            # 音频
            AudioMaterial = draft.AudioMaterial(os.path.join(f"{self.tts_dir}/wenan.mp3"))
            audio_length = AudioMaterial.duration
            print(audio_length)
            self.script.add_material(AudioMaterial)
            AudioSegment = draft.AudioSegment(AudioMaterial,
                            trange(0, int(audio_length)),
                            volume=1)
            AudioSegment.add_effect(AudioSceneEffectType.空灵感)
            self.script.add_segment(AudioSegment, 'TTS')
            self.audioNowTime += audio_length
            ...
            video_segment = draft.VideoSegment(material = video_material,
                                                            target_timerange  = trange(0, self.audioNowTime),
                                                            volume=0,
                                                            clip_settings=draft.ClipSettings(transform_y=-0.70, transform_x=-0.4, scale_x=0.05, scale_y=0.05))
            video_segment.add_animation(IntroType.渐显, 600000)
            video_segment.add_animation(OutroType.渐隐, 600000)
            self.script.add_segment(video_segment, 'SY')
            ...
            TextSegment = draft.TextSegment(self.title, trange(0, self.audioNowTime),  # 文本将持续整个视频（注意script.duration在上方片段添加到轨道后才会自动更新）
                            ...
                            style=draft.TextStyle(color=(1, 1, 1)),                # 设置字体颜色为黄色
                            border=draft.TextBorder(alpha=0.2,color=(0, 0, 0)),
                            clip_settings=draft.ClipSettings(transform_x=0,transform_y=-0.70, scale_x=0.45, scale_y=0.45))          # 模拟字幕的位置
            # TextSegment.add_animation(GroupAnimationType.哈哈镜, 2000000)
            self.script.add_segment(TextSegment, 'TITLE')

    def general_draft(self):
        try:
            self.addTitle(title=self.title, wenan=self.wenan)
            # self.addItem()
            # self.addBgm()
