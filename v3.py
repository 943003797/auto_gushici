...
from src.agents.agent_v3 import agent_v3 as ag
from src.video_processing.autocut.auto_cut_v3 import auto_cut_v3 as act
import os, json, shutil
import asyncio
from pathlib import Path
from src.video_processing.autocut.auto_cut_v3 import autoCut

def create_ui():
    with gr.Blocks(title="语录开头古诗词短视频 V3") as demo:
        gr.Markdown("# 语录开头古诗词短视频 V3")
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                poetry = gr.Textbox(label="开头文案", lines=20, type="text")

                # Confirm poetry button
                confirm_copy_btn = gr.Button("确认文案")
                confirm_poetry = gr.Textbox(label="诗词", lines=20)
            ...
            with gr.Column(scale=1):
                ...
                generate_tts_btn.click(
                    fn=generate_tts,
                    inputs=[title_input, poetry, confirm_poetry, reference_audio_dropdown],
                ...
                regenerate_tts_btn.click(
                    fn=regenerate_tts,
                    inputs=[tts_dropdown, title_input, poetry, confirm_poetry, reference_audio_dropdown],
                    outputs=[tts_dropdown]
                )
        ...
        def generateDraft(title: str, wenan: str, confirm_poetry: str, bgm: str, bgv: str):
            act = autoCut(title=title, wenan=wenan, list=json.loads(confirm_poetry), bgm=bgm, bgv=bgv)
            return act.general_draft()
        gr.Button("Go").click(
            fn=generateDraft,
            inputs=[title_input, poetry, confirm_poetry, bg_audio_dropdown, bg_video_dropdown]
        )
